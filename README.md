# Banco Central de la República Dominicana Exchange Rates API — bcrd-exchange-rate
...
> **Official rate, not mid-market:** every value here is a number Banco Central de la República Dominicana itself published, fixed once printed and carrying the central bank's own `rate_date` — what filings and audits require. Need the live interbank midpoint for pricing or display instead? Use the [mid-market API](https://allratestoday.com/docs/) or [`@allratestoday/sdk`](https://www.npmjs.com/package/@allratestoday/sdk). The two can diverge by several percent.

## ⚡ Try it without a key

The latest Banco Central de la República Dominicana table is also served keyless, CORS-open and edge-cached, for evaluation, embeds and AI agents:

```bash
curl "https://allratestoday.com/api/open/central-bank/bcrd?source=USD&target=DOP"
```

```js
const r = await fetch('https://allratestoday.com/api/open/central-bank/bcrd').then((x) => x.json());
...
Requires Node 18+ (global `fetch`); also runs on Bun, Deno and edge runtimes. Also published under the org scope as [`@allratestoday/bcrd-exchange-rate`](https://www.npmjs.com/package/@allratestoday/bcrd-exchange-rate) — same code, same versions.
...
- Publisher's own page: [Tasas de cambio del mercado spot](https://www.bancentral.gov.do/a/d/2537-mercado-cambiario) · [www.bancentral.gov.do](https://www.bancentral.gov.do)
- Publication: every business day; the exact schedule, freshness status and any current delay are on the [Banco Central de la República Dominicana rates page](https://allratestoday.com/central-bank-rates-api/bcrd/)
...
- Banco Central de la República Dominicana quotes **DOP per 1 unit of foreign currency** (e.g. `base: "USD", quote: "DOP"` means DOP per one US dollar).
...
Loading the official Banco Central de la República Dominicana rate into an accounting system is a supported workflow, not a hack. Step-by-step guides with the direction each system expects:
...
```bash
curl "https://allratestoday.com/api/v1/central-bank/bcrd/latest?format=xml&api_key=art_live_..."
```
...
- [Package docs on the site](https://allratestoday.com/docs/sdk/bcrd-exchange-rate/) · [ERP integration guides](https://allratestoday.com/docs/integrations/)
